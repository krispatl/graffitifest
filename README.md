...
**Live style + FX:** Select a style and press MORPH STYLE to blend the current name into it over three seconds. SURPRISE ME chooses a different style. This preserves the piece, seed, queue, phase timer, and HOLD state; it does not change the default style for future names. Wait for one morph to finish before triggering another. PULSE gently scales the paint, WAVE bends it, GLITCH shifts horizontal strips, and SPRAY boosts aerosol particles for four seconds. One effect runs at a time; another trigger replaces/restarts it. STOP FX cancels the effect immediately. Morph and effects can run together and during HOLD. They require an active visible piece before its exit transition. Effects use persisted timestamps, so a reconnect resumes only the remaining animation, never an expired trigger. Blackout hides output while these short effect clocks continue. The static Canvas2D fallback shows the resulting style but cannot animate live effects.
