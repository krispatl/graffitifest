...
import { animationAt, liveAnimationAt } from './AnimationDirector';
import { createParticles } from './ParticleSystem';

const fragment = `
precision highp float;
varying vec2 vUv;
uniform sampler2D uPreviousArt,uPreviousDetail; uniform float uMorph,uEffect,uEffectTime,uEffectAmount;
...
void main(){
 ...
 if(uEffect==1.)uv=(uv-.5)/(1.+.075*uEffectAmount*(.5-.5*cos(uEffectTime*3.14159)))+.5;
 if(uEffect==2.)uv.x+=sin(uv.y*19.-uEffectTime*4.)*.028*uEffectAmount;
 if(uEffect==3.)uv.x+=(noise(vec2(floor(uv.y*32.),floor(uEffectTime*9.)))-.5)*.12*uEffectAmount;
 vec2 warp=vec2(sin(uv.y*17.+uMorph*6.),cos(uv.x*13.-uMorph*4.))*.035*sin(uMorph*3.14159);
 float morphBlend=smoothstep(0.,1.,uMorph);
 ...
 vec4 art=texture2D(uArt,uv-warp);vec4 previous=texture2D(uPreviousArt,uv+warp);
 art=vec4(mix(previous.rgb*previous.a,art.rgb*art.a,morphBlend),mix(previous.a,art.a,morphBlend));art.rgb/=max(art.a,.0001);art*=step(0.,uv.x)*step(uv.x,1.)*step(0.,uv.y)*step(uv.y,1.);
 ...
 vec4 detail=texture2D(uDetailArt,uv-warp);vec4 previousDetail=texture2D(uPreviousDetail,uv+warp);
 detail=vec4(mix(previousDetail.rgb*previousDetail.a,detail.rgb*detail.a,morphBlend),mix(previousDetail.a,detail.a,morphBlend));detail.rgb/=max(detail.a,.0001);float detailReveal=smoothstep(noise(uv*8.)-.04,noise(uv*8.)+.04,uDetail);color=mix(color,detail.rgb,detail.a*detailReveal*step(0.,uv.x)*step(uv.x,1.)*step(0.,uv.y)*step(uv.y,1.));
 if(uEffect==4.){float dust=pow(noise(uv*150.+uEffectTime*.3),9.);color+=uColor*dust*uEffectAmount*.9;}
 if(t>0.){
...
export class GraffitiRenderer {
  ...
  private previousTexture: THREE.CanvasTexture | null = null;
  private previousDetail: THREE.CanvasTexture | null = null;
  private id = '';
  private aspect = 16 / 9;
  private sizeKey = '';
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.material = new THREE.ShaderMaterial({
      uniforms: {
        uPreviousArt: { value: null },
        uPreviousDetail: { value: null },
        uMorph: { value: 1 },
        uEffect: { value: 0 },
        uEffectTime: { value: 0 },
  ...
  render(s: Snapshot, now: number) {
    ...
    const fx = liveAnimationAt(s, now);
    const renderKey = `${s.current.id}:${s.current.morph?.id ?? ''}`;
    if (this.id !== renderKey) {
      ...
      this.previousDetail?.dispose();
      this.previousTexture = null;
      this.previousDetail = null;
      if (s.current.morph && fx.morph < 1) {
        const previous = generateGraffiti(
          { ...s.current, settings: s.current.morph.fromSettings },
          this.aspect,
        );
        this.previousTexture = new THREE.CanvasTexture(previous.canvas);
        this.previousDetail = new THREE.CanvasTexture(previous.detailCanvas);
        this.previousTexture.colorSpace = THREE.NoColorSpace;
        this.previousDetail.colorSpace = THREE.NoColorSpace;
      }
      this.material.uniforms.uPreviousArt.value = this.previousTexture ?? this.texture;
      this.material.uniforms.uPreviousDetail.value = this.previousDetail ?? this.detailTexture;
      ...
      if (!this.id.startsWith(s.current.id + ':')) this.origin = performance.now();
      this.id = renderKey;
    }
    const a = animationAt(s, now),
      u = this.material.uniforms,
      cal = s.settings.calibration;
    if (fx.morph >= 1 && this.previousTexture) {
      u.uPreviousArt.value = this.texture;
      u.uPreviousDetail.value = this.detailTexture;
      this.previousTexture.dispose();
      this.previousDetail?.dispose();
      this.previousTexture = this.previousDetail = null;
    }
    u.uMorph.value = fx.morph;
    u.uEffect.value = fx.effect;
    u.uEffectTime.value = fx.elapsed;
    u.uEffectAmount.value = fx.amount;
    ...
    this.particles.material.uniforms.uAmount.value =
      Math.min(1, s.settings.particles / 100 + (fx.effect === 4 ? fx.amount : 0)) *
      (1 - a.transition);
    this.particles.material.uniforms.uDrawing.value =
      s.phase === 'DRAWING' ? 1 : fx.effect === 4 ? fx.amount * 2 : 0;
